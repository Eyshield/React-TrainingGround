...
const useEditTask = () => {
  ...
  const handleEditTask = async (id: string, taskRequest: TaskRequest) => {
    setLoading(true);
    setError(null);

    try {
      const response = await updateTask(id, taskRequest);
...
function AddTask({ projectId, onClose, onSuccess, id }: EditTaskProps) {
  ...
  useEffect(() => {
    if (taskData) {
      ...
      setForm({
        ...
        userId: String(taskData.username),
        projectId: projectId,
        columnId: "", // i didnt still decided how to get it
      });
      setSelectedUser({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await handleEditTask(id, taskRequest);
      ...
      // L'erreur est déjà gérée par useEditTask
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Mettre a jour une tâche à votre projet.
            </p>
          </div>
